...
func (h *smokeHandler) OnRemoteStopTransaction(request *core.RemoteStopTransactionRequest) (*core.RemoteStopTransactionConfirmation, error) {
	fmt.Println("ChargePoint received AUTO RemoteStopTransaction:", "transactionId:", request.TransactionId)
...
func main() {
	clientID := env("CLIENT_ID", "CP-LIMIT-AUTO-001")
	...
	bootConf, err := cp.BootNotification("LimitSmokeModel", "LimitSmokeVendor")
	if err != nil {
	...
	fmt.Println("Waiting for server-origin AUTO RemoteStopTransaction. No manual REST stop will be sent.")

	remoteStopReq := waitRemoteStop(handler.remoteStopCh, 20*time.Second)
	...
	fmt.Println("StopTransaction after auto-stop:", stopStatus)
	...
	fmt.Println("StatusNotification: Available after auto-stop")

	fmt.Println("pure auto-stop smoke complete")
}
...
func waitRemoteStop(ch <-chan *core.RemoteStopTransactionRequest, timeout time.Duration) *core.RemoteStopTransactionRequest {
	...
	case req := <-ch:
		return req
	case <-time.After(timeout):
		log.Fatalf("timed out waiting for AUTO RemoteStopTransaction")
	}
